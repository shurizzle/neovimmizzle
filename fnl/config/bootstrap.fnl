...
(if (= (vim.fn.has :nvim-0.9.0) 0)
    (do
      (git-clone "https://github.com/lewis6991/impatient.nvim" :impatient.nvim)
      (xpcall (fn [] (require :impatient))
              (fn []
                (vim.api.nvim_echo [["Error while loading impatient" :ErrorMsg]]
                                   true []))))
    (vim.loader.enable))
...
(let [fennel (require :hotpot.fennel)
  ...
  (set fennel.path (table.concat [(join-paths base :?.fnl)
...
(fn additional-macros []
  ...
  (f.eval (slurp (join-paths (vim.fn.stdpath :config) :fnl :config
                             :init-macros.fnl))
...
(do
  (fn build [files]
    ...
    (local allowedGlobals
           (vim.tbl_keys (collect [n _ (pairs _G)]
                           (values (global-unmangling n) true))))
    (build (vim.fn.stdpath :config)
           {:verbose true
            :atomic true
            :force true
            :compiler {:modules {: allowedGlobals :correlate true : useBitLib}}}
           files))
