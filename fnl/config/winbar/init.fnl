...
(fn redraw-status [winnr]
  (vim.schedule (fn []
                  (pcall vim.api.nvim_win_call winnr
...
(fn unset-winbar [winnr]
  (when (not (empty? (vim.api.nvim_get_option_value
                       :winbar {:scope :local :win winnr})))
...
(fn options [?opts]
  ...
  (set opts.buftype (or opts.buftype
                        (vim.api.nvim_buf_get_option opts.buf :buftype)
                        ""))
  (set opts.filetype (or opts.filetype
                         (vim.api.nvim_buf_get_option opts.buf :filetype)
                         ""))
...
(fn setup []
  ...
  (vim.api.nvim_create_autocmd
    ...
    {:callback (fn [{: buf : file}]
                 (when (and (vim.api.nvim_buf_is_valid buf) (not (empty? file)))
  ...
  (each [_ winid (ipairs (vim.api.nvim_list_wins))]
    (when (vim.api.nvim_win_is_valid winid)
      ...
      (when (vim.api.nvim_buf_is_valid bufnr)
        (xpcall
          #(vim.api.nvim_win_call winid #(buf-changed {:buf bufnr}))
          #(vim.api.nvim_echo [[(if (not (string? $1)) (vim.inspect $1) $1)
                                :ErrorMsg]]
                              true []))))))
