(fn add-parsers []
  (let [config ((. (require :nvim-treesitter.parsers) :get_parser_configs))]
    (set config.prolog {:install_info {:url "https://github.com/Rukiza/tree-sitter-prolog"
                                       :files [:src/parser.c]
                                       :revision :ee1c64cc15e96430c51914dfee7ca205159b0586}
                        :filetype :prolog})
    (set config.pezzo {:install_info {:url "https://github.com/shurizzle/tree-sitter-pezzo"
                                      :files [:src/parser.c]
...
(fn build []
  (add-parsers)
  (((. (require :nvim-treesitter.install) :update) {:with_sync (. (require :config.platform)
                                                                  :is :headless)})))

(fn config []
  (add-parsers)
  ((. (require :nvim-treesitter.configs) :setup) {:ensure_installed []
                                                  :sync_install (. (require :config.platform)
                                                                   :is :headless)
